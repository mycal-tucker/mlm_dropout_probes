...
class SimpleDataset:
  ...
  """
    ...
    '''
        ...
        train_observations = self.optionally_add_embeddings(train_observations, train_embeddings_path, break_on_qmark=self.args['dataset']['embeddings']['break_on_qmark'])
        dev_observations = self.optionally_add_embeddings(dev_observations, dev_embeddings_path, break_on_qmark=self.args['dataset']['embeddings']['break_on_qmark'])
        test_observations = self.optionally_add_embeddings(test_observations, test_embeddings_path, break_on_qmark=self.args['dataset']['embeddings']['break_on_qmark'])
    ...
    '''
        ...
        for index in tqdm(sorted([int(x) for x in indices]), desc='[aligning embeddings]'):
            ...
            if break_on_qmark and '?' in observation.sentence:  # Question-answer case!
                ...
                context = line_str[q_mark_idx + 2:]
                question = line_str[:q_mark_idx + 1]
            ...
            # print("Features had shape", single_layer_features.shape)
            # print("Tokenized shape", len(tokenized_sent))
            assert single_layer_features.shape[0] == len(tokenized_sent)
            single_layer_features = torch.tensor(
                np.array([np.mean(single_layer_features[untok_tok_mapping[i][0]:untok_tok_mapping[i][-1] + 1, :], axis=0) for i
                 in
                 range(len(untokenized_sent))]))
            assert single_layer_features.shape[0] == len(observation.sentence)
            single_layer_features_list.append(single_layer_features.float())
